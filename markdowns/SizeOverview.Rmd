  ...
  for (j in 1:length(years)){
    ...
    if (nrow(Ydata) > 0) {
      ...
      result <- summary %>% ggplot(aes(x = Date, y = Relative_abundance, fill = SizeClass))+
        ...
        labs(x = "Date", y = "Relative abundance (%)", title = paste0(lakes[[i]], " in ", years[[j]]))+
        theme_minimal()
...
for (i in 1:length(lakes)){
  ...
  for (j in 1:length(years)){
    ...
    if (nrow(Ydata) > 0) {
      ...
      result <- Pdata %>% ggplot(aes(x = Date, y = `Abundance_ind/L`, fill = SizeClass))+
        geom_col(color = "Black", linewidth = 0.05)+
        labs(x = "Date", y = "Abundance ind/L", title = paste0(lakes[[i]], " in ", years[[j]]))+
        theme_minimal()
