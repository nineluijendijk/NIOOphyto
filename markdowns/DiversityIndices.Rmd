...
Mdata <- Ydata %>% group_by(Date, Species, LakeName) %>% summarize(Count = sum(Counts, na.rm = TRUE))

summary <- Mdata[c("Date", "Species", "Count")] %>% group_by(Date) %>% summarize(richness = specnumber(Count),
